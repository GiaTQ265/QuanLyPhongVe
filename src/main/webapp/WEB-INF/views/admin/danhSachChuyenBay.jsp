						...
						<section id="unseen">
							...
							<div class="pagination">
								<c:if test="${currentPage > 1}">
									<a
										href="${pageContext.request.contextPath}/admin/chuyen-bay/danh-sach-chuyen-bay?page=${currentPage-1}">Previous</a>
								</c:if>
								<c:if test="${1 lt (currentPage-10)}">
									<a
										href="${pageContext.request.contextPath}/admin/chuyen-bay/danh-sach-chuyen-bay?page=${currentPage-10}">...</a>
								</c:if>
								<c:if test="${(currentPage+10) lt totalPages}">
									<c:forEach begin="${currentPage}" end="${currentPage+10}"
										...
										<c:choose>
											...
											<c:otherwise>
												<a
													href="${pageContext.request.contextPath}/admin/chuyen-bay/danh-sach-chuyen-bay?page=${i}">${i}</a>
											</c:otherwise>
										</c:choose>
									</c:forEach>
								</c:if>
								...
								<c:if test="${currentPage lt totalPages}">
									<a
										href="${pageContext.request.contextPath}/admin/chuyen-bay/danh-sach-chuyen-bay?page=${currentPage+1}">Next</a>
								</c:if>
							</div>
						</section>
					</div>
