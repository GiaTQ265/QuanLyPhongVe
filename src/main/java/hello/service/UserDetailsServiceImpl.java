...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String soDienThoai) throws UsernameNotFoundException {
		...
		final TaiKhoan taiKhoan = taiKhoanRepository.timTheoSoDT(soDienThoai);
		
		// Kiểm tra có tồn tại không và ném ra exception
		if (taiKhoan == null) {
			throw new UsernameNotFoundException(soDienThoai);
		}
		...
		GrantedAuthority grantedAuthority = new SimpleGrantedAuthority(taiKhoan.getVaiTro());
		...
		UserDetails userDetails = (UserDetails) new User(taiKhoan.getSoDienThoai(), taiKhoan.getMatKhau(), grantedList);
		return userDetails;
	}
