						...
						<section>
							...
							<ul class="pagination" style="float: right;">
								<c:if test="${currentPage > 1}">
									<li><a
										href="${pageContext.request.contextPath}/admin/ve/timkiem?searchKey=${key}&page=${currentPage-1}">Trang
											trước</a></li>
								</c:if>
								<c:forEach begin="1" end="${totalPages}" var="i">
									<c:choose>
										...
										<c:otherwise>
											<li><a href="${pageContext.request.contextPath}/admin/ve/timkiem?searchKey=${key}&page=${i}">${i}</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								<c:if test="${currentPage lt totalPages}">
									<li><a
										href="${pageContext.request.contextPath}/admin/ve/timkiem1?searchKey=${key}&page=${currentPage+1}">Tiếp
											theo</a></li>
								</c:if>
							</ul>
						</section>
